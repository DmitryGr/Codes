...
int n = 9;
vector<int> a = {3, 4, 5, 6, 7, 6, 7, 8, -1}; // i -> a[i] в 0-индексации, -1 => нет ребра
vector<int> b = {1, 2, 4, 5, 8, 7, 8, -1, -1}; // i -> b[i] в 0-индексации, -1 => нет ребра
...
int main(){
  ...
  for (int mask = 0; mask < (1<<n); ++mask) {
    ...
    if (is_mask_good(mask)) {
      ...
      //per(cnt);
      if (cnt == 4) {
        int x = mask & ((1<<(n - 1)) + (1<<(n-2)));
        assert((mask&x) == 0); //если размер маски 4 и она хорошая, в ней не должно быть n и n-1;
      }
    }
  }
